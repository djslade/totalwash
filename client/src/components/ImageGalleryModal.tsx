...
export const ImageGalleryModal = () => {
    ...
    const handlePreviousPhotoClick = () => {
        const currentIndex = snap.currentProduct.photos.indexOf(snap.currentGalleryPhoto)
        const targetIndex = currentIndex === 0 ? snap.currentProduct.photos.length - 1 : currentIndex - 1
        state.currentGalleryPhoto = snap.currentProduct.photos[targetIndex]
    }

    const handleNextPhotoClick = () => {
        const currentIndex = snap.currentProduct.photos.indexOf(snap.currentGalleryPhoto)
        const targetIndex = currentIndex === snap.currentProduct.photos.length - 1 ? 0 : currentIndex + 1
        state.currentGalleryPhoto = snap.currentProduct.photos[targetIndex]
    }
            ...
            className={`z-[100] top-0 left-0 right-0 bottom-0 flex justify-center items-center fixed`}>
                <div className="flex flex-col p-6 h-full items-center gap-3 justify-center" ref={modalRef}>
                    ...
                    <div className="max-w-xl flex justify-center">
                        ...
                        className="text-gray-50 text-4xl absolute left-3 top-1/2 -translate-y-1/2"><AiOutlineLeft /></button>
                        <img src={snap.currentGalleryPhoto} className="w-full"/>
                        <button
                        onClick={handleNextPhotoClick}
                        className="text-gray-50 text-4xl absolute right-3 top-1/2 -translate-y-1/2"><AiOutlineRight /></button>
                    </div>
                    ...
                    <div className="grid-cols-6 gap-3 grid max-w-xl">
                        ...
                        key={photo}
                        className={`w-full pb-3 hover:border-b border-gray-300  ${photo === snap.currentGalleryPhoto? "border-b !border-gray-50" : ""}`}>
                            <img className="w-full aspect-square object-cover" src={photo} alt={snap.currentProduct.name}/>
                        </button>)}
                    </div>
                </div>
            </motion.div>
